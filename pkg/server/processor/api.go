...
import (
	...
	"dedicated-container-ingress-controller/pkg/server/handler"
...
func NewAPI(settings APISettings) (*API, error) {
	...
	router.PathPrefix("/").Handler(
		handler.NewDedicatedContainerHandler(
			settings.DedicatedContainerFactory,
			settings.RedisPool,
			core.NewSessionRouter(&sessions.CookieStore{
				Codecs: securecookie.CodecsFromPairs([]byte(settings.CookieSecretKey)),
